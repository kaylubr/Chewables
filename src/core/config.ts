...
function parseAppEnv(raw: string | undefined): AppEnv {
	...
	throw new Error(
		`Invalid NODE_ENV "${raw}"; expected ${Object.values(APP_ENVS).join(", ")}`,
	);
}
...
export const config = {
	...
	mail: {
		...
		from: process.env.MAIL_FROM ?? 'Chewables <verify@chewable.app>',
	},

	corsOrigin:
		env === APP_ENVS.production
			? undefined
