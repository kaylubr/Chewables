/**
 * Service-layer behavior tests: frame validation, storage-key generation,
 * upload validation. Ported from the Python/pytest suite.
 */
import { describe, it, expect } from 'vitest';
import { randomUUID } from 'node:crypto';
import { isSupportedFrame, FRAME_IDS } from '../lib/frames.js';
import {
	InvalidImageError,
	MAX_UPLOAD_BYTES,
	...
	storageKeyFor,
	validateUpload,
} from '../domains/photos/photos.service.js';
...
describe('upload validation', () => {
	...
	it('rejects empty and oversized', () => {
		...
		expect(() => validateUpload('image/png', MAX_UPLOAD_BYTES + 1)).toThrow(InvalidImageError);
	});
});
